    ...
    this.subscriptions.push(
      this.productDetail.productDetails$.subscribe(detail => {
        if (detail && detail.productImage.length > 0) {
          ...
          this.isActive = [];
          this.isActive[detail.productImage[0].productId] = true;
          for (let i = 0; i < imageLength; i++) {if (detail.productImage[i].defaultImage === 1) {
            this.productImageId = detail.productImage[0].productId;
            this.image =
            this.imagePath + '?path=' +
                detail.productImage[0].containerName + '&name=' +
                detail.productImage[0].image +
                '&width=390&height=390';
               // this.image = detail.productImage[0].containerName + detail.productImage[0].image;
              this.zoomImage =
              this.imagePath + '?path=' +
              detail.productImage[i].containerName + '&name=' +
              detail.productImage[i].image +
              '&width=660&height=660';
             // this.zoomImage =
            //  '/assets/images/'+detail.productImage[0].containerName + detail.productImage[0].image;;
            this.changeDetectRef.detectChanges();
            setTimeout(() => {
              this.config.observer = true;
              this.changeDetectRef.detectChanges();
